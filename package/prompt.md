...
```
# 方案 ：补齐 extract-src 工程配置并重新构建 cli-dev.js

## 目标
让 `extract-src` 成为可独立二次开发、可重新构建出 `cli-dev.js` 的完整工程,要求 `cli-dev.js` 和 `cli.js` 的执行效果一致。

## 现状与风险

1. **无工程配置**：`extract-src` 缺少 `package.json`、`tsconfig.json`、构建脚本。
2. **`node_modules` 残缺**：当前 `node_modules` 只有 source map 反解时顺带提取出的**部分文件**（如 `chalk` 只有 `source/`、`react` 只有 `cjs/`），缺少 `package.json`，Bun 无法解析。
3. **绝对路径别名丢失**：源码中约有 1000+ 处 `import ... from 'src/...'`，原工程通过 `tsconfig.json` 的 `baseUrl`+`paths` 解析，提取后全部失效。
4. **React Compiler 产物**：源码中存在 750+ 处 `import { c as _c } from "react/compiler-runtime"`，但该模块不存在于 `node_modules` 中。npm 上有独立的 `react-compiler-runtime` 包，需要通过 alias 映射。
5. **Bun 专有 API**：20+ 个文件使用 `import { feature } from 'bun:bundle'`，只有 `bun build` 能正确解析；直接 `bun run` 调试时会报错（如需要可额外做 stub 兼容）。
6. **Native 模块**：`extract-src` 中没有 `.node` 文件，运行时通过 `vendor/` 下的 TS wrapper 去加载外层 `vendor/audio-capture/*.node`。重新打包后若仍把产物放在项目根目录，相对路径通常仍可找到原 native 模块。

## 实施步骤

### Phase 1：创建工程配置文件

**1.1 创建 `extract-src/package.json`**
- `type: "module"`
- `scripts`：
  - `"build": "bun build src/main.tsx --outfile ../cli-dev.js --target node --sourcemap"`
  - `"dev": "bun run src/main.tsx"`（可选，用于快速验证，但受 `bun:bundle` 限制）
- `dependencies`：根据源码 import 扫描整理出所有**包级依赖**（如 `lodash-es`、`chalk`、`zod`、`axios`、`react`、`@anthropic-ai/sdk`、`@modelcontextprotocol/sdk` 等），注意子路径（如 `lodash-es/memoize.js`）不需要单独列包名，只需列根包 `lodash-es`。
- 额外加入 `react-compiler-runtime`，用于映射 React Compiler 产物中的 `react/compiler-runtime` 引用。

**1.2 创建 `extract-src/tsconfig.json`**
- `compilerOptions.baseUrl: "."`
- `paths`：
  - `"src/*": ["./src/*"]` —— 恢复原始绝对路径别名。
  - `"react/compiler-runtime": ["./node_modules/react-compiler-runtime"]` —— 解决 React Compiler 产物 import 问题。
- `jsx: "react-jsx"`（源码为 `.tsx`，需要 JSX transform）。
- `moduleResolution: "bundler"`（与 Bun 构建器兼容）。

### Phase 2：重建依赖环境

**2.1 清理残缺 `node_modules`**
```bash
cd extract-src
rm -rf node_modules
```
...
**2.2 安装完整依赖**
- 根据 1.1 中的依赖列表执行 `bun install`。
- 重点验证以下包是否安装完整（有 `package.json` 且能正常 resolve）：
  - `chalk`、`react`、`lodash-es`、`zod`、`axios`、`@anthropic-ai/sdk`、`@modelcontextprotocol/sdk`、`react-compiler-runtime`

### Phase 3：处理 `bun:bundle` 与 Native 模块兼容

**3.1 `bun:bundle`**
- `bun build` 原生支持 `bun:bundle`，构建时无需改动源码。
- 若需要直接 `bun run` 做开发调试，可在 `extract-src/src` 下创建一个 `bun-bundle-stub.ts`，导出 `feature`（返回 `null` 或相应 mock），并通过 `tsconfig.json` 的 `paths` 将 `bun:bundle` 映射到该 stub。此步骤为可选，视调试需求决定。

**3.2 Native 模块路径验证**
- 检查 `extract-src/vendor/*/index.ts` 中 `require('*.node')` 的相对路径。
- 确认构建产物 `cli-dev.js` 输出到项目根目录后，这些相对路径是否仍能指向外层 `vendor/` 中的 `.node` 文件。如有偏移，通过 symlink 或复制 `vendor/` 目录到 `extract-src/` 内修正。
...
**4.2 排错与修复**
- 若 Bun 报 `Could not resolve "src/..."`，检查 `tsconfig.json` 的 `paths` 是否被 Bun 正确读取（必要时在 build 命令中加 `--tsconfig-override`）。
- 若报 `Could not resolve "react/compiler-runtime"`，检查 `react-compiler-runtime` 是否正确安装，以及 `tsconfig.json` alias 是否生效。
- 若报缺少某依赖，将该依赖补入 `package.json` 重新安装。

**4.3 运行验证**
```bash
bun run ../cli-dev.js --version
# 或
node ../cli-dev.js --version
```
- 验证基本启动是否正常。
- 如有 Native 模块加载错误，回到 Phase 3.2 调整路径。

### Phase 5：建立可持续开发工作流

- 修改 `extract-src/src/` 中的源码。
- 运行 `bun run build` 重新生成根目录的 `cli-dev.js`。
- 用 `bun run ../cli-dev.js` 运行验证修改效果。

## 预期交付物

1. `extract-src/package.json`
2. `extract-src/tsconfig.json`
3. 完整可用的 `extract-src/node_modules`
4. 成功重新构建出的 `cli-dev.js`
